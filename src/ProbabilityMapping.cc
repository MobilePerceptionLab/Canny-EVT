        ...
        for(size_t y = 0; y< (size_t)kf->im_.rows; y++) {
            for (size_t x = 0; x < (size_t) kf->im_.cols; x++) {
                ...
                if (kf->depth_map_checked_.at<float>(y,x) > 0.000001) {
                    ...
                    fileOut << "l " + std::to_string(num) + " " + std::to_string(num+1) << std::endl;
                    num = 2 + num;
                }
            }
        }
...
std::tuple<Eigen::Vector3f,Eigen::Vector3f> ProbabilityMapping::Cal3dGradient(ORB_SLAM2::KeyFrame* kf, int x, int y){
    //get 3d points in the world
    Eigen::Vector3f P_3d(kf->SemiDensePointSets_.at<float>(y, 3 * x),
                       kf->SemiDensePointSets_.at<float>(y, 3 * x + 1),
                       kf->SemiDensePointSets_.at<float>(y, 3 * x + 2));
    //get camere position in the world
    cv::Mat Twc_ = kf->GetPoseInverse();
    cv::Mat Pose_cam = Twc_.rowRange(0,3).col(3).clone();
    Eigen::Vector3f Pose_camera(Pose_cam.at<float>(0,0), 
                                Pose_cam.at<float>(1,0),
                                Pose_cam.at<float>(2,0));
    std::cout<<Pose_cam.at<float>(1,0)<<" "<<Pose_cam.at<float>(0,1);
    std::cout<<Pose_cam.at<float>(2,0)<<" "<<Pose_cam.at<float>(0,2);
    //get the surface normal
    Eigen::Vector3f surface_normal(P_3d(0) - Pose_camera(0),
                                   P_3d(1) - Pose_camera(1),
                                   P_3d(2) - Pose_camera(2));
    //get the 2d gradient
    float gx = kf->Gradx.at<float>(y, x);
    float gy = kf->Grady.at<float>(y, x);
    //set Z1 and Z2 and get 2 points in the space. the 2d point is from the gradient direction
    float Z1 = 1;
    float Z2 = 2;

    float X1 = Z1 *((x+gx)- kf->cx ) / kf->fx;
    float Y1 = Z1 *((y+gy)- kf->cy ) / kf->fy;

    cv::Mat Pc1 = (cv::Mat_<float>(4,1) << X1, Y1 , Z1, 1); // point in camera frame.
    cv::Mat Twc = kf->GetPoseInverse();
    cv::Mat pos1 = Twc * Pc1;

    float X2 = Z2 *((x+gx)- kf->cx ) / kf->fx;
    float Y2 = Z2 *((y+gy)- kf->cy ) / kf->fy;

    cv::Mat Pc2 = (cv::Mat_<float>(4,1) << X2, Y2 , Z2, 1); // point in camera frame.
    cv::Mat pos2 = Twc * Pc2;
    ...
    float Xg = pos1.at<float>(0) + ray(0) * t;
    float Yg = pos1.at<float>(1) + ray(1) * t;
    float Zg = pos1.at<float>(2) + ray(2) * t;
    Eigen::Vector3f Pg(Xg, Yg, Zg);
    return make_tuple(Pg,surface_normal);
}
